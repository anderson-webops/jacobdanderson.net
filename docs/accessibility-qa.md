...
This checklist complements the automated `npm run a11y` axe smoke suite. Run it before shipping portfolio, project, resume, classes, and contact changes.

## Screen Reader Pass

- macOS VoiceOver: open `/`, `/projects`, `/experience`, `/resume`, `/contact`; use `VO + Right Arrow` through each page, then jump by headings, landmarks, buttons, form controls, and links.
...
- Verify visible focus on header navigation, project links, resume links, class links, contact form fields, buttons, and footer links.
